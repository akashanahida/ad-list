...
class RuleValidator:
    ...
    def _valid_adblock(self, rule: str) -> bool:
        # 0. Check for DOMAIN-SUFFIX/DOMAIN format first
        if RE_DOMAIN_SUFFIX_LIST.match(rule):
            m = RE_DOMAIN_SUFFIX_LIST.match(rule)
            if m:
                domain = m.group(1)
                # Basic validation: not IP-like and has valid chars
                if RE_IP_LIKE.match(domain):
                    return False
                return True
...
def compress_rules(rules: List[str], include_wildcards: bool, include_regex: bool = False) -> Tuple[List[str], List[str]]:
    ...
    for line in rules:
        ...
        for r in parsed_list:
            if r.can_compress and r.hostname:
                ...
                is_regex = r.rule_text.startswith('/') and r.rule_text.endswith('/')
                
                if include_wildcards and '*' in r.rule_text and not is_cmt:
                    kept_wildcards.append(r.rule_text)
                elif include_regex and is_regex and not is_cmt:
                    kept_regex.append(r.rule_text)
                else:
                    filtered_text.append(r.original_text)
    ...
    merged_output = final_list + kept_wildcards + kept_regex
    strictly_validated_output: List[str] = []

    for rule in merged_output:
        # Regex rules (starting with /) bypass the output filter
        is_regex = rule.startswith('/') and rule.endswith('/')
        if is_regex or RE_OUTPUT_FILTER.match(rule):
...
def main() -> int:
    ...
    parser.add_argument("--include-regex", action="store_true", help="Keep regex rules (starting with /)")
    ...
    # 3. Compression
    compressed, compression_filtered = compress_rules(valid_content, args.include_wildcards, args.include_regex)
